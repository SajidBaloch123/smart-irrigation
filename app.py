...
with col_input:
    st.subheader("🌾 Real-Time Field Telemetry")
    crop_type = st.selectbox("Target Crop", ["Wheat", "Rice", "Cotton", "Maize", "Vegetables"])
...
with col_output:
    st.subheader("📊 Automated Operations & Agronomist Logs")
    if execute_btn:
        if not groq_api_key:
            ...
            with st.spinner("Analyzing telemetry data..."):
                ...
                with metric_col1:
                    st.metric(label="Moisture Deficit", value=f"{deficit_pct}%")
                with metric_col2:
                    st.metric(label="Calculated Volume Dosage", value=f"{water_volume} L/m²")
        ...
        st.write("*Awaiting telemetry data... Adjust parameters and click compute.*")
